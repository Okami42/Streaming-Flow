...
export default function SeriesEpisodesPage({ params }: PageProps) {
  ...
  useEffect(() => {
    const loadSeries = async () => {
      try {
        ...
        const foundSeries = seriesData.find((item) => 
          item.id === id || id.startsWith(item.id + "-")
        );
  ...
  const getEpisodesToDisplay = () => {
    if (hasMultipleSeasons) {
      const season = series.seasonsList?.find((s: any) => s.seasonNumber === selectedSeason);
    ...
    <div className="flex flex-col min-h-screen bg-[#030711]">
      ...
      <main className="flex-grow">
        <div className="container mx-auto px-4 py-8">
          <div className="mb-8">
            <Link href={`/series/${seriesId}`} className="inline-flex items-center text-blue-400 hover:text-blue-300 mb-4">
                ...
                <div className="relative inline-block">
                  ...
                  {isSeasonMenuOpen && (
                    <div className="absolute z-10 mt-1 w-48 bg-[#151a2a] rounded-md shadow-lg py-1 border border-white/10">
                      {series.seasonsList?.map((season: any) => (
                        <button
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
            {getEpisodesToDisplay().map((episode: Episode, index: number) => {
              let duration = "55 min";
              ...
              return (
                <EpisodeCard
                  key={`${seriesId}-${selectedSeason}-${episode.id}`}
                  id={episode.id}
                  title={title}
