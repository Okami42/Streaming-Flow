...
export default function DebugEpisodePage() {
  ...
  const testFileAccess = async () => {
    const url = '/anime_episodes_js/shingeki-no-kyojin/saison1/episodes_vostfr.js';
    
    try {
      console.log('🔍 Test accès fichier:', url);
      setResult(prev => prev + `🔍 Test accès fichier: ${url}\n`);
  ...
  return (
    <div className="p-6 max-w-4xl mx-auto">
      <h1 className="text-2xl font-bold mb-4">🔧 Debug Accès Fichiers</h1>
      
      <button
        onClick={testFileAccess}
        className="px-4 py-2 bg-blue-500 text-white rounded hover:bg-blue-600 mb-4"
      >
        🧪 Tester Accès Fichier
      </button>
