...
export default function AnimeEpisodesPage({ params }: PageProps) {
  ...
  // État pour l'anime enrichi
  const [anime, setAnime] = useState<Anime | null>(null);
  const [loading, setLoading] = useState(true);
  
  // Charger l'anime avec auto-import
  useEffect(() => {
    const loadAnime = async () => {
      try {
        ...
        if (enrichedAnime) {
          ...
          // Fallback vers la version basique si auto-import échoue
          const basicAnime = getAnimeById(animeId);
          setAnime(basicAnime || null);
        }
      } catch (error) {
...
>>>>>>> 975648a9ed94bfc5da709a184d4431ff9a4d85b3
} 