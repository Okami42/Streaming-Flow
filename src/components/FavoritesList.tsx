          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="mt-4 flex justify-end">
              {item.seriesId && (
                <Link 
                  href={item.episodeId 
                    ? `/catalogue/${item.seriesId}/watch/${item.episodeId}${item.seasonNumber ? `?season=${item.seasonNumber}` : ''}`
                    : `/catalogue/${item.seriesId}${item.seasonNumber ? `?season=${item.seasonNumber}` : ''}`
                  }
                >
                  <Button 
                    variant="outline" 
                  ...
                  >
                    {item.episodeId ? 'Regarder l\'épisode' : 'Voir l\'animé'}
